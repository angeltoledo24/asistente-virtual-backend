...
class AudioUploadView(APIView, IAServices):
    ...
    def post(self, request, format=None, ):
        ...
        # })
        completion = ia_services.gpt_normal(text_input=text_convert, context=context, id=id)
...
def get_audio_response(request, id):
    if request.method == 'GET':
        ...
        if os.path.exists(filepath_response) and os.path.exists(filepath_input):
            ...
            print(f"El archivo {filepath_response} y {filepath_input} ha sido eliminado exitosamente.")
        else:
